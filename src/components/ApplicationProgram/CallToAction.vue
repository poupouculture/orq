<template>
  <div class="w-full grid grid-cols-12 bg-gray-200 ml-4 rounded-md p-4 gap-4">
    <div class="col-span-4 flex flex-col">
      ...
      <InputSelect :options="actionTypeOptions" default="Call Phone Number" :value="actionType"
        @input="updateActionType" class="bg-white rounded-lg" />
    </div>
    <div class="col-span-2 flex flex-col">
      <span class="font-semibold">
        Button Text
      </span>
      <input type="text" class="w-full h-10 block border rounded-lg pl-4" v-model="actionText"
    ...
    <div class="col-span-3 flex flex-col">
      ...
      <InputSelect :options="actionType === at.CALL_PHONE ? actionCountryOptions : actionWebtypeOptions"
        :default="actionType === at.CALL_PHONE ? 'ID +62' : 'Static'" :value="actionCountryOrWebtype"
        @input="updateActionCountryOrWebtype" class="bg-white rounded-lg" />
    </div>
    <div class="col-span-3 flex flex-col">
      <span class="font-semibold">
        {{ actionType === at.CALL_PHONE ? 'Phone Number' : 'URL' }}
      </span>
      <input type="text" class="w-full h-10 block border rounded-lg pl-4" v-model="actionValue"
        @change="handleAllChange" />
    </div>
  </div>
</template>
