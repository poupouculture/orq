<template>
  ...
  <AddUserGroupOverlay
    ...
    :data="userGroupData"
    :selected-data="selectedUserGroup"
...
import { addCustomerGroup, updateCustomerGroup } from "src/api/customerGroup";
...
const selectedCustomer = ref(
  data.value?.customers.map((item) => item.customers_id.id) || []
);
const selectedUserGroup = ref(
  data.value?.user_groups.map((item) => item.user_groups_id) || []
);
